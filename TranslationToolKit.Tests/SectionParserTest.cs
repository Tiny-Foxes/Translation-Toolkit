...
namespace TranslationToolKit.Tests
{
    public class SectionParserTest
    {
        ...
        public void WhenProvidingWithAListOfLinesThenParseItIntoASection()
        {
            ...
            {
                Assert.Contains(key, section.Lines[key].Text);
                Assert.Equal(key, section.Lines[key].Title);
            }
        }
        ...
        public void WhenProvidedWithASimpleSectionThenParseTheSection()
        {
            ...
            Assert.Equal(new Line("Fitness", "Fitness=Fitness Mode"),section.Lines["Fitness"]);
            Assert.Equal(new Line("HelpText", "HelpText=&BACK; Exit &START; Select &MENUUP;&MENUDOWN; Move"), section.Lines["HelpText"]);
            Assert.Equal(new Line("Network OK", "Network OK=Network OK"), section.Lines["Network OK"]);
            Assert.Equal(new Line("Offline", "Offline=Offline"), section.Lines["Offline"]);
            Assert.Equal(new Line("Connected to %s", "Connected to %s=Connected to %s"), section.Lines["Connected to %s"]);
            Assert.Equal(new Line("CurrentGametype", "CurrentGametype=Current Gametype"), section.Lines["CurrentGametype"]);
            Assert.Equal(new Line("LifeDifficulty", "LifeDifficulty=Life Difficulty"), section.Lines["LifeDifficulty"]);
            Assert.Equal(new Line("TimingDifficulty", "TimingDifficulty=Timing Difficulty"), section.Lines["TimingDifficulty"]);
            Assert.Equal(new Line("%i Songs (%i Groups), %i Courses", "%i Songs (%i Groups), %i Courses=%i Songs (%i Groups), %i Courses"), section.Lines["%i Songs (%i Groups), %i Courses"]);
        }
        ...
        public void WhenProvidedWithCommentsThenTheyAreAddedToTheNextLine()
        {
            ...
            Assert.Equal(new Line("Text1", "Text1=WWWWWWWWWWWW", "# W is annoying:"), section.Lines["Text1"]);
            Assert.Equal(new Line("Text2", "Text2=The argument goes something like this: &oq;I refuse to prove that I exist,&cq; says God, &oq;for proof denies faith, and without faith I am nothing.&cq;::::&oq;But,&cq; says Man, &oq;The Babel fish is a dead giveaway, isn't it? It could not have evolved by chance. It proves you exist, and so therefore, by your own arguments, you don't. QED.&cq;::::&oq;Oh dear,&cq; says God, &oq;I hadn't thought of that,&cq; and promptly vanished in a puff of logic.", "# Realistic excerpt:"), section.Lines["Text2"]);
            Assert.Equal(new Line("Text3", "Text3=01234567890 ::100.00% 12345% 9:00 50/100", "# Numbers test:"), section.Lines["Text3"]);
            Assert.Equal(new Line("Text4", "Text4=$50% (Test) \"test's\" &oq;test's&cq;, #1! 1+2-3*4/5.6::3:00 a; b; 5<6 6>5  2^3=8 Why? test@example.com::[Foo] {bar} Press &START; 3|4 &oq;1_2_3&cq;", "# General symbol test, and test alignment against another font."), section.Lines["Text4"]);
            Assert.Equal(new Line("Text5", "Text5="), section.Lines["Text5"]);
        }
        ...
        public void WhenProvidedWithEmptyLineThenTheyAreAddedToTheSection()
        {
            ...
            Assert.Equal(new Line("HeaderText", "HeaderText=Practice Songs"), section.Lines["HeaderText"]);
            Assert.Equal(new Line($"{Section.EmptyLineTitlePrefix}0", ""), section.Lines[$"{Section.EmptyLineTitlePrefix}0"]);
            Assert.Equal(new Line("ExplanationGroup", "ExplanationGroup=Select the song group."), section.Lines["ExplanationGroup"]);
            Assert.Equal(new Line("ExplanationSong", "ExplanationSong=Select the song you wish to practice."), section.Lines["ExplanationSong"]);
            Assert.Equal(new Line("ExplanationAction", "ExplanationAction=Press Start to practice!"), section.Lines["ExplanationAction"]);
            Assert.Equal(new Line("ExplanationStepsType", "ExplanationStepsType=Select the StepsType you want to practice."), section.Lines["ExplanationStepsType"]);
            Assert.Equal(new Line("ExplanationSteps", "ExplanationSteps=Select a notechart."), section.Lines["ExplanationSteps"]);
        }
